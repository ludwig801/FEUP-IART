...
public class Minimax : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _distanceToNextRow = new int[_gameManager.NumPlayers];
        _distanceToObjective = new int[_gameManager.NumPlayers];
        _weights = new float[NumEvalFeatures, _gameManager.NumPlayers];
    ...
    {
        // First Player
        var objective = _gameManager.GetPlayerTile(0).row + 1;
        var retVal = new int[_gameManager.NumPlayers];
        ...
        // Second Player
        objective = _gameManager.GetPlayerTile(1).row - 1;
    ...
    int AStar(int player, int objective, bool contemplatePawns)
    {
        ...
        var start = _gameManager.GetPlayerTile(player);
        ...
        while (openList.Count > 0)
        {
            ...
            for (int i = 0; i < bestChoice.neighbors.Count; i++)
            {
                var neighbor = bestChoice.neighbors[i];
    ...
    {
        return (player == 0 ? _gameManager.Pawns[0].Tile.row : _gameManager.GameBoard.Size - 1 - _gameManager.Pawns[1].Tile.row);
    }
